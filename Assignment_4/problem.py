...
class Server:
    ...
    def update_storage(self, msg):
        ...
        try:
            ...
            #print("payload (after decryption):", payload)
...
# def find_original_sk_bytes(ecdsa_sk, N):
#     for i in range(1 << 16):  # 小优化：尝试低 16-bit 变动（快速）
#         candidate = ecdsa_sk + i * N
#         try:
#             sk_bytes = candidate.to_bytes(16, "big")
#             if int.from_bytes(sk_bytes, "big") % N == ecdsa_sk:
#                 return sk_bytes
#         except:
#             continue
#     return None
...
if __name__ == "__main__":
    ...
    # print("--- ---")
    # print("Pro ecdsa_sk: ", server.ecdsa_sk)
    # print("Pro static_sk: ", server.static_sk.data)
    # # Use the P256 curve
    # curve = get_curve("secp256r1")
    # # N is the order of the group
    # N = curve.field.n
    # temp = find_original_sk_bytes(server.ecdsa_sk, N)
    # #temp = server.ecdsa_sk.to_bytes(16, 'big')
    # temp_sk = PrivateKey(temp + b'\x00' * 16)
    # print("test static_sk: ", temp_sk.data)
